...
//
// The client never sees R2 credentials. It calls this function with its
// Supabase JWT; the function verifies the user, then hands back a short-lived
// presigned URL so the browser uploads/downloads bytes DIRECTLY to/from R2.
//
...
const URL_TTL_SECONDS = 300; // presigned URLs live 5 minutes
...
const CORS = {
  "Access-Control-Allow-Origin": "*", // TODO: lock to the app origin before launch
...
// Keep object keys safe and namespaced to the owner.
...
Deno.serve(async (req) => {
  ...
  let payload: { action?: string; filename?: string; contentType?: string; key?: string };
  try {
  ...
  if (payload.action === "download") {
    ...
    // A user may only fetch objects under their own prefix.
    if (!payload.key.startsWith(prefix)) return json({ error: "forbidden" }, 403);
