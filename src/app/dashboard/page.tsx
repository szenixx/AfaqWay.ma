...
/* Placeholder post-auth landing. Confirms auth works end to end; the real
   dashboard/onboarding is a later build. */
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "var(--paper)" }}>
      ...
      <main style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div className="card" style={{ width: "100%", maxWidth: 440, textAlign: "center" }}>
          <span className="pill pill-indigo">Dashboard</span>
          {loading ? (
            <p style={{ font: "400 15px/24px var(--font-sans)", color: "var(--ink-soft)", margin: "16px 0 0" }}>Loading your session…</p>
          ) : email ? (
            <>
              <h1 style={{ font: "700 24px/32px var(--font-sans)", color: "var(--ink)", margin: "16px 0 0" }}>You are signed in</h1>
              <p style={{ font: "400 14px/22px var(--font-sans)", color: "var(--ink-soft)", margin: "8px 0 0" }}>
                Signed in as <strong style={{ color: "var(--ink)" }}>{email}</strong>. This is a placeholder dashboard, the real onboarding comes next.
              </p>
