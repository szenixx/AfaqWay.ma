...
export const ACCENTS = ["#2E3BC7", "#20C997", "#F76707", "#845EF7", "#F06595", "#0CA678"];
...
export function StatCard({ label, value, prefix, suffix, delta, icon, loading, accent = "#2E3BC7" }: { label: string; value: number; prefix?: string; suffix?: string; delta?: number; icon?: ReactNode; loading?: boolean; accent?: string }) {
  ...
  return (
    <div className="dash-card" style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "clamp(9px,1.3vh,15px) clamp(14px,1.5vw,20px)", background: "linear-gradient(120deg, rgba(59,91,219,.14), rgba(32,201,151,.10) 60%, rgba(132,94,247,.12))" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
        <UserAvatar size={48} user={{ id: me?.id, name: me?.name ?? "Super Admin" }} />
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 3 }}>
            <span style={{ font: "600 9.5px/1 var(--font-sans)", letterSpacing: ".06em", textTransform: "uppercase", color: "#fff", background: "linear-gradient(135deg,#2E3BC7,#845EF7)", borderRadius: 999, padding: "3px 8px" }}>Super Admin</span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, flex: "none" }}>
        ...
        <div style={{ textAlign: "right" }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "flex-end", gap: 6 }}>
            <span style={{ font: "800 clamp(24px,3.6vh,38px)/1 var(--font-sans)", letterSpacing: "-.5px", background: "linear-gradient(135deg,#2E3BC7,#20C997)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent", fontVariantNumeric: "tabular-nums" }}>{time || "—"}</span>
