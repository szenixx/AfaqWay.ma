...
function Row({ choice, selected, showTick }: { choice: Choice; selected: boolean; showTick?: boolean }) {
  return (
    <>
      {choice.flag
        ? <Flag stripes={choice.flag} size="md" unavailable={choice.disabled} selected={selected} style={{ flex: "none" }} />
        : choice.bars
      ...
      <span className="onb-opt-text">
        <span className="onb-opt-label">{choice.label}</span>
        {choice.sub && <span className="onb-opt-sub">{choice.sub}</span>}
      </span>
      {choice.hint ? (
        <span className="onb-opt-side">{choice.hint}</span>
      ) : showTick && selected ? (
...
function ChoiceRows({ control, value, label, onChange }: { control: Extract<Control, { kind: "choice" }>; value: string; label: string; onChange: (v: string) => void }) {
  return (
    <RadioGroup className="onb-opts" data-cols={control.columns} data-compact={control.compact || undefined} aria-label={label} value={value} onChange={onChange}>
...
function MultiRows({ control, value, label, onChange }: { control: Extract<Control, { kind: "multi" }>; value: string; label: string; onChange: (v: string) => void }) {
  ...
  return (
    <CheckboxGroup
      className="onb-opts" data-cols={control.columns} aria-label={label}
      value={chosen}
...
function Dropdown({ control, value, label, emoji, onChange }: { control: Extract<Control, { kind: "select" }>; value: string; label: string; emoji?: EmojiName; onChange: (v: string) => void }) {
  return (
    <Select
      className="onb-select" aria-label={label} placeholder={control.placeholder}
      value={value || null}
...
export function Answer({
  control, value, label, emoji, invalid, onChange, onCommit, onEnter,
}: {
  ...
  if (control.kind === "date") {
    return (
      <div className="onb-inputbox" data-invalid={invalid || undefined}>
        {emoji && <Emoji name={emoji} size={22} className="onb-input-emoji" />}
        <TextField className="onb-field" aria-label={label} type="date" value={value} onChange={onChange} isInvalid={invalid}>
          <Input className="onb-input" onBlur={onCommit} onKeyDown={enterKey} />
        </TextField>
      </div>
    );
  }
  ...
  return (
    <div className="onb-inputbox" data-invalid={invalid || undefined}>
      {emoji && <Emoji name={emoji} size={22} className="onb-input-emoji" />}
      <TextField className="onb-field" aria-label={label} value={value} isInvalid={invalid}
        onChange={(v) => onChange(sanitize(control, v))}>
        <Input
          className="onb-input" placeholder={control.placeholder} inputMode={control.inputMode}
          maxLength={control.maxLength} onBlur={onCommit} onKeyDown={enterKey} autoFocus
        />
      </TextField>
    </div>
  );
  ...
  return (
    <div className="onb-inputbox onb-phone" data-invalid={invalid || undefined}>
      ...
      <TextField className="onb-phone-code" value={code} onChange={(v) => onCode(v.replace(/[^\d+]/g, "").slice(0, 5))}>
          <Label className="onb-sr">Country dialling code</Label>
      ...
      <TextField className="onb-phone-number" value={number} isInvalid={invalid} onChange={(v) => onNumber(v.replace(/[^\d]/g, "").slice(0, 15))}>
        <Label className="onb-sr">WhatsApp number</Label>
